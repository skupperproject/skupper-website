...
def update_openapi(output_dir="input"):
    ...
    Reads version from input/data/install.json and fetches the OpenAPI spec
    from the skupperproject/skupper repo to input/api/
    """
    output_dir = get_absolute_path(output_dir)
    ...
    make_dir(join(output_dir, "api"))
    write(join(output_dir, "api", "openapi.yaml"), http_get(url))
