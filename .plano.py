...
@command
def update_docs(output_dir="doc-input"):
    """
    Update the v2 documentation from skupper-docs repo

    This fetches the latest docs and places them in doc-input/ for MkDocs to build.
    """
    output_dir = get_absolute_path(output_dir)
    url = "https://github.com/pwright/skupper-docs/archive/vale.tar.gz"
    ...
    with temp_file() as temp:
        ...
        with working_dir(quiet=True):
            ...
            # Replace doc-input/ with fresh content from skupper-docs
            replace(output_dir, join(extracted_dir, "input"))

@command
def update_openapi(docs_dir="doc-input"):
    """
    Update the network-observer OpenAPI spec for v2 docs

    Reads version from input/data/install.json and writes OpenAPI spec to doc-input/api-docs/
    """
    docs_dir = get_absolute_path(docs_dir)
    # Version data is generated by generate_releases in input/data
    version = read_json("input/data/install.json")["version"]
    url = f"https://raw.githubusercontent.com/skupperproject/skupper/{version}/cmd/network-observer/spec/openapi.yaml"

    make_dir(join(docs_dir, "api-docs"))
    write(join(docs_dir, "api-docs", "openapi.yaml"), http_get(url))
...
def render_docs():
    ...
    This renders doc-input/ to output/docs/ with sidebar navigation.
    It does NOT touch output/v1/ which is rendered by Transom.
    """
    notice("Installing MkDocs dependencies")
    run("pip install -q mkdocs-material mkdocs-macros-plugin mkdocs-awesome-pages-plugin")
